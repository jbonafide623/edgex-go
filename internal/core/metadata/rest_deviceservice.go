...
func restAddDeviceService(w http.ResponseWriter, r *http.Request) {
	...
	if ds.Id, err = dbClient.AddDeviceService(ds); err != nil {
		HttpErrorHandler.Handle(w, err, []httperror.ErrorConceptType{httperror.DatabaseNotUniqueErrorConcept{}}, httperror.StatusInternalServerErrorConcept{})
		return
	}
...
func updateDeviceServiceFields(from models.DeviceService, to *models.DeviceService, w http.ResponseWriter) error {
	...
	if from.Name != "" {
		...
		if err != nil {
			HttpErrorHandler.Handle(w, err, []httperror.ErrorConceptType{httperror.DeviceServiceDuplicateErrorConcept{CurrentDSId: checkDS.Id, UpdatedDSId: to.Id}}, httperror.StatusServiceUnavailableErrorConcept{})
		}
	}
